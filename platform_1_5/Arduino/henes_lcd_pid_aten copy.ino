...
int St_M = 512; // 플랫폼 직진 조향값 (200~800) 영진전문대:500 제주도1호기:610 제주도2호기:450 제주도3호기:520 
int St_D = 370; // 플랫폼 조향 최대 중립 차이  영진전문대:300 제주도 1호기:420  제주도 2호기:420 이전값 : 380 충남대:420 우리:380
int Max_Speed = 250; // 플랫폼의 최대 속도값 (0~255)
byte Gear; // 플랫폼 기어 명령값
byte AorM; // 플랫폼 자율주행 모드, 수동주행 모드, 디버깅 모드 명령값
byte ESTOP; // 플랫폼 긴급정지 명령값
int lsp1,lsp2,lsp3,lsp4,lsp5 = 0;// 모터 속도 이동 평균 제어154
...
void controller(){ // 수신기 데이터를 받는 함수
  ...
  */
  ...
  // steer 신호 처리
  if(steer < 1530 and steer > 1430){ // steer 신호의 DEADZONE
    steer = 1450;
  }
  else{
    steer = steer;
