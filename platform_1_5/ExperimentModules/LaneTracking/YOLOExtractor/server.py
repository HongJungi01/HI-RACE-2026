...
def tcp_server():
    ...
    try:
        while True:
            ...
            try:
                while True:
                    # 1. 헤더 8바이트 수신 (Signature 4 + Length 4)
                    header = recvall(conn, 8)
                    if not header or header[:4] != b'IMG!': 
                        break
                    
                    img_len = struct.unpack('>I', header[4:])[0]
                    ...
                    # 3. AI 연산 수행
                    try:
                        result_list = main_module.main(bytes(img_bytes), *PARAMS)
                        
                        if result_list:
                            # 결과 리스트를 문자열로 직렬화 ("val1|val2|...")
                            ...
                            print(f"[{addr}] Processed: {res_str}")
