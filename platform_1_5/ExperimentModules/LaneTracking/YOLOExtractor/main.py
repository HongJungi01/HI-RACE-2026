...
def main(Image_String, min_area, min_span, max_rmse, poly_degree=2):
    ...
    left_lane_base64 = encode_image_to_base64(left_mask)
    right_lane_base64 = encode_image_to_base64(right_mask)
    ...
    return [
        str(heading),           # [0] 헤딩 에러
        str(cte),               # [1] CTE
        left_lane_base64,       # [2] 왼쪽 차선 이미지 (Base64)
        right_lane_base64,      # [3] 오른쪽 차선 이미지 (Base64)
        debug_img_base64,       # [4] 전체 디버그 이미지 (Base64)
        f"Total: {total_time_ms:.1f}ms" # [5] 시간 정보
    ]